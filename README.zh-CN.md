...
AuraTask 把研究生日常会用到的几件事——任务规划、专注计时、习惯养成、思路整理、状态记录——揉成一个轻量原生应用。数据全部存在本地 SQLite，没有账号、没有云、不联网。

## 特性

- 任务管理：日 / 周 / 月三种粒度自由切换，任务可标记**主要 / 延伸**两种类型，周视图按上午 / 下午 / 晚上色块分区；
- 稍后待做：无日期任务暂存区，等空了再排期；
- 番茄钟计时：专注 / 休息时长可自定义，可绑定一个任务做"我正在专注于……"，结束自动入库并弹 Windows 系统通知；
- 习惯养成：支持二元习惯（做没做）和量化习惯，并显示连续天数**、**近 30 天完成率、迷你热力图；
- 简单的思维导图：左→右自动布局的树状导图，多张导图可命名管理，节点可**关联任务**，自动显示完成状态；
